...
//   setClientKeyPair(KeyPair.PRIVATE_KEY);
//   setServerPublicKey(KeyPair.PUBLIC_KEY);
//   const dom = new JSDOM(``);
...
//   const nonce = 'TjBU0kAoEV5wx2n+JaRIPg=='
...
test("client RSAEncrypt/RSADecrypt Test", async () => {
  ...
  global.window.crypto = new Crypto();
  const data = "0000";
  let result = await RSAEncrypt(KeyPair.PUBLIC_KEY, str2ab(data));
  let res = await RSADecrypt(KeyPair.PRIVATE_KEY, base64Str2ab(result));
  expect(data).toBe(ab2str(res));
});